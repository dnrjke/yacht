...
프론트엔드가 Socket.io로 백엔드와 실시간 통신합니다. 턴 진행·이벤트 검증·점수 계산은 서버 권위이지만, 쏟기 물리 시뮬레이션과 최종 주사위 눈은 클라이언트 권위입니다 — 턴 플레이어가 로컬에서 Rapier를 실행하고 결과를 서버에 전송하면, 서버는 구조적 sanity check만 수행한 뒤 상대에게 릴레이합니다. 자동 플레이 시에만 서버가 물리를 직접 실행합니다.
...
LOBBY (스플래시 / Touch to Start)
  -> MAIN_MENU (Single Play, Local Play, Online, Settings)
    ...
    -> ONLINE_LOBBY (방 생성 또는 코드 참가 -> 상대 대기)
      -> GAME (온라인: 서버 권위 턴 진행 + 클라이언트 권위 물리)
        -> GAME_OVER
```
...
- 온라인 관전 시 모바일에서 렉 발생
- 자동 플레이(연결 끊김 대행) 동작이 불안정
- 턴 플레이어가 빠르게 조작하면 관전자 측 연출이 꼬임 (이벤트 순서 가드 부재)
